       	...
       	<div class="form-body mt-4">
		    <div class="row">
			   	<div class="col-lg-12">
					   ...
					   <div class="mb-3">
							<label for="inputclient_name" class="form-label">Client Name</label>
							<select class ="form-control" name="client_name" id="client_name" disabled="true">
						...
						<div class="mb-3">
							...
							<label for="inputproject_name" class="form-label">Project Name</label>
							<?php  $errorClass =  !empty($errors->has('project_name')) ? 'is-invalid':''; ?>   
			                {!! Form::text('project_name', null, array('id'=>'project_name','placeholder' => 'Enter Project Name','readonly'=>'readonly', 'class' => 'form-control '.$errorClass )) !!}
			                {!! !empty($errors->has('project_name')) ?'<div class="invalid-feedback"><span>'.$errors->first('project_name').'</span></div>' :'' !!}
						</div>
						<div class="mb-3">
							<label for="inputteam_member" class="form-label">Team member</label>
							<?php  $errorClass =  !empty($errors->has('team_member')) ? 'is-invalid':''; ?> 
							<select class ="form-control skills" name="team_member[]" multiple='multiple' disabled="true">
						...
						<div class="mb-3">
							<label for="inputproject_award_date" class="form-label">Project Award Date</label>
							<?php  $errorClass =  !empty($errors->has('project_award_date')) ? 'is-invalid':''; ?>   
			                {!! Form::date('project_award_date', null, array('id'=>'project_award_date','placeholder' => 'Enter Date', 'readonly'=>'readonly','class' => 'form-control '.$errorClass )) !!}
			                {!! !empty($errors->has('project_award_date')) ?'<div class="invalid-feedback"><span>'.$errors->first('project_award_date').'</span></div>' :'' !!}
						</div>
						<div class="mb-3">
							
							
							<label for="inputtotal_amount" class="form-label">Total Amount</label>
							<?php  $errorClass =  !empty($errors->has('total_amount')) ? 'is-invalid':''; ?>   
			                {!! Form::text('total_amount', null, array('id'=>'total_amount','placeholder' => 'Enter Total Amount','id'=>'pointspossible','readonly'=>'readonly', 'class' => 'form-control '.$errorClass )) !!}
			...
			<div class="row">
				...
				@if(count($data['milestone_details']) > 0)
					...
					<input type="text" class="form-control  milestone_name" name="milestone_name[]" id="milestone_name" placeholder="Please enter milestone name" value="{{$data['milestone_name'][$i]}}" readonly>			
					@endfor
				    @endif
				</div>
					<div class="col-sm mb-3">
					...
					@if(count($data['milestone_details']) > 0)
					@for($i = 0; $i < count($data['milestone_details']); $i++)
					<input type="text" class="form-control milestone_details" name="milestone_details[]" id="milestone_details" placeholder="Please enter milestone details" value="{{$data['milestone_details'][$i]}}" readonly>
					@endfor
				    @endif
					</div>
					<div class="col-sm mb-3">
					...
					<input type="text" class="form-control pointsperc" name="percentage_project_amount[]" placeholder="Enter % of project amount" value="{{$data['percentage_project_amount'][$i]}}" readonly>
					@endfor
				    @endif
					</div>
					<div class="col-sm mb-3">
					...
					<input type="text" class="form-control amount" name="totalproject_amount[]" placeholder="Total amount" value="{{$data['totalproject_amount'][$i]}}" readonly>
					@endfor
				    @endif
					</div>
					<div class="col-sm mb-3">
					...
					<input type="date" class="form-control" name="due_date[]" placeholder="Please enter due date" value="{{$data['due_date'][$i]}}" required="required">
					@endfor
				    @endif
					</div>
